...
function drawBackground(W, H, ground) {
  ...
  ctx.save();
  if (backgroundImage) {
    ctx.filter = 'blur(' + Number($('bgBlur').value) + 'px) brightness(' + Number($('bgBright').value) + '%)';
    const iw=backgroundImage.naturalWidth, ih=backgroundImage.naturalHeight;
    const fit=$('bgFit').value;
    let dw=W0,dh=H0,dx=0,dy=0;
    if(fit==='cover'){const s=Math.max(W0/iw,H0/ih);dw=iw*s;dh=ih*s;dx=(W0-dw)/2;dy=(H0-dh)/2}
    else if(fit==='contain'){const s=Math.min(W0/iw,H0/ih);dw=iw*s;dh=ih*s;dx=(W0-dw)/2;dy=(H0-dh)/2}
    ctx.drawImage(backgroundImage,dx,dy,dw,dh);
    ctx.filter='none';
  } else {
    const g=ctx.createLinearGradient(0,0,0,H0);
    g.addColorStop(0,'#15304b'); g.addColorStop(.55,'#7193b1'); g.addColorStop(1,'#b9c5d0');
    ctx.fillStyle=g; ctx.fillRect(0,0,W0,H0);
    ctx.fillStyle='rgba(255,255,255,.12)'; ctx.beginPath(); ctx.arc(W0*.78,H0*.18,H0*.16,0,Math.PI*2); ctx.fill();
  }
  ctx.restore();
  ctx.fillStyle='rgba(255,255,255,.18)'; ctx.fillRect(0,ground,W0,H-ground);
  ctx.strokeStyle='rgba(30,45,60,.65)'; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(0,ground); ctx.lineTo(W0,ground); ctx.stroke();
  if($('grid').checked){
    ctx.strokeStyle='rgba(35,55,75,.20)'; ctx.lineWidth=1;
    for(let cm=0;cm<=250;cm+=25){const y=ground-(cm/Math.max(250,maxHeight()))*(H0*.70);if(y<0)continue;ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(W0,y);ctx.stroke();ctx.fillStyle='rgba(20,35,50,.7)';ctx.font='10px system-ui';ctx.textAlign='left';ctx.fillText(cm+' cm',8,y-3)}
  }
}onst $ = id => document.getElementById(id);
const canvas = $('c');
const ctx = canvas.getContext('2d');
